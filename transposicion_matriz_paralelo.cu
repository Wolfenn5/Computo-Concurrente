...
__global__ void transponerMatriz(float *matrizA, float *matrizTranspuesta, int n) 
{
    ...
    if (fila<n && columna<n)  // si el id hilo en eje x ó y (fila, columna) es menor al numero total de datos (N) hacer el procesamiento
    {
        ...
        matrizTranspuesta[columna*n+fila]= matrizA[fila*n+columna]; //se invierte la fila y la columna de cada indice de las matrices porque eso es lo que se busca, que se inviertan (transposicion)
    }
}
...
int main(int argc, char const *argv[]) 
{
    ...
    float *matrizTranspuesta_host= (float*) malloc(sizeof(float)*N*N);



    // Declarar matrices en el dispositivo (GPU)
    float *matrizA_dispositivo, *matrizTranspuesta_dispositivo;
    cudaMalloc(&matrizA_dispositivo, N*N* sizeof(float));
    cudaMalloc(&matrizTranspuesta_dispositivo, N*N* sizeof(float));
    ...
    // Lanzar el kernel
    transponerMatriz<<<tamanio_malla, tamanio_bloque>>>(matrizA_dispositivo, matrizTranspuesta_dispositivo, N);
    ...
    cudaMemcpy(matrizTranspuesta_host, matrizTranspuesta_dispositivo, sizeof(float)*N*N, cudaMemcpyDeviceToHost);
    ...
    //     for (int j=0; j<N; j++) 
    //     {
    //         printf("%f ", matrizTranspuesta_host[i*N+j]);
    //     }
    //     printf("\n");
    // }
    // printf("\n%f\n", matrizTranspuesta_host[(N*N)-1]); // ultimo elemento nadamas (para probar con valores grandes cuando se hace con numeros consecutivos)
    ...
    cudaFree(matrizTranspuesta_dispositivo);


    // Liberar memoria del host (CPU)
    free(matrizA_host);
    free(matrizTranspuesta_host);
