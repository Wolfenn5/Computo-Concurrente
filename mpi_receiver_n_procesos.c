...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        MPI_Get_processor_name(nombre_receptor,&longitud);
        MPI_Send(&nombre_receptor,MPI_MAX_PROCESSOR_NAME,MPI_CHAR,0,0,MPI_COMM_WORLD); //variable que se va a enviar, el tamaño de la variable, tipo caracter de MPI, identificador del proceso, etiqueta del mensaje
        printf("\nEnviamos el mensaje %s desde el proceso %d al proceso %d\n",nombre_receptor,rank,0);
    }
