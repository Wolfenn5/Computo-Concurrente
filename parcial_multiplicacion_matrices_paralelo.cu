...
__global__ void MultiplicarMatrices(float *matrizA, float *matrizB, float *matrizC, int n) 
{
    ...
    if (fila<n && columna<n)  // si el id hilo en eje x ó y (fila, columna) es menor al numero total de datos (N) hacer el procesamiento
    {
        ...
        for (int i=0; i<n; i++) 
        {
            ...
            // se usa el numero de filas como base para el desplazamiento (fila*n+i)
            suma+= matrizA[fila*n+i] * matrizB[i*n+columna];
        }
