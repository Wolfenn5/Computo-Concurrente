...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        MPI_Send(&nombre_host,MPI_MAX_PROCESSOR_NAME,MPI_CHAR,1,0,MPI_COMM_WORLD); //variable que se va a enviar, el tamaño de la variable, tipo caracter de MPI, identificador del proceso, etiqueta del mensaje
        printf("\nEnviamos el mensaje %s desde el proceso %d\n",nombre_host,rank);
    }
    else
    {
